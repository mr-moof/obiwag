<#
.SYNOPSIS
    Integration smoke test: re-runs the driver-hype fork-and-strip scenario
    against a fresh fixture, asserting all five /obi-auto-max gates fire.

.DESCRIPTION
    Acceptance criteria from plan (Issue 161 final):
...
Describe 'fork-and-strip smoke (all 5 gates)' {
    ...
    BeforeEach {
        $script:Fixture = Join-Path $TestDrive 'fixture-driver-hype'
...
traces = [
    ('runner', 'no active runners online with tags x'),
    ('quota',  'monthly minutes quota exceeded'),
    ('yaml',   'jobs:test config contains unknown keys'),
...
"@
        ...
        $combined -match 'runner=runner-tag-mismatch' | Should Be $true
