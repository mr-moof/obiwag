...
.DESCRIPTION
    ...
    tool can't run in Pester); simulates one reversible auto-default and one
    material user answer by passing a hashtable to the write-back logic.

    Contract: `policies/rigor-max-gates.md` ("Phase 0 - Prereq Lock-In"); plan-file schema:
    `policies/obi-auto-max-schema.md`.

    Acceptance criteria:
    - Phase 0 auto-locks a reversible declared default without a continuation prompt
    - Phase 0 retains AskUserQuestion only for a material input (mocked here)
...
"@
    ...
    Context 'Plan-file write-back simulation' {
        ...
        It 'replaces placeholders with answers and appends runtime.phase0 block' {
            # Simulate the orchestrator's write-back: it auto-locks the safe declared
            # default, asks only for the free-text material input, then uses Edit to
            # swap placeholders. Verify the WRITTEN result matches the spec.

            $simulatedAnswers = @(
                @{ id = 'target_namespace'; answer = 'user-personal'; locks_field = 'bootstrap-config.json:repoBase'; source = 'auto-default' }
                @{ id = 'runner_tags';      answer = 'custom-build';   locks_field = ''; source = 'AskUserQuestion' }
            )
...
"@
            foreach ($a in $simulatedAnswers) {
                ...
                $writeback += "`n        source: `"$($a.source)`""
            }
            $writeback += "`n    peer:"
            $writeback += "`n      status: `"unavailable`""
            $writeback += "`n      provider: `"codex`""
            $writeback += "`n      reason: `"test fixture - peer not exercised`""
            ...
            $final -match 'user-personal' | Should -Be $true
            $final -match 'custom-build' | Should -Be $true
            ...
            $final -match 'source: "auto-default"' | Should -Be $true
            $final -match 'source: "AskUserQuestion"' | Should -Be $true
            $final -match 'peer:' | Should -Be $true
            $final -match 'provider: "codex"' | Should -Be $true
    ...
    Context 'Idempotence on re-run' {
        ...
        It 'orchestrator detects existing runtime.phase0 block and skips re-locking' {
            # After Phase 0 has run once, re-running /obi-auto-max must NOT
            # re-lock defaults, re-ask material inputs, or duplicate the runtime.phase0 block.
            # The orchestrator (policies/rigor-max-gates.md, Phase 0 step 2) detects
            # this and skips ahead to probes + peer-on-plan.
            $simulatedFinal = @"
phase0:
  - id: target_namespace
    placeholder: "<target-namespace>"
    options: [user-personal]
...
"@
            ...
            # Idempotence-guard contract (per policies/rigor-max-gates.md, Phase 0 step 2):
