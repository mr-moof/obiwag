<#
.SYNOPSIS
    Pester 3.4 tests for tools/probes/runner_tags.ps1.
...
Describe 'runner_tags probe' {
    ...
    Context 'GitHub runners mapping (stubbed gh)' {
        ...
        It 'collects sorted-unique label names and reports shared_runners false when runners exist' {
            Set-GhStub -ExitCode 0 -Stdout '{"total_count":2,"runners":[{"id":1,"name":"w1","status":"online","labels":[{"id":1,"name":"self-hosted","type":"read-only"},{"id":2,"name":"pscodesign","type":"custom"}]},{"id":2,"name":"w2","status":"online","labels":[{"id":3,"name":"self-hosted","type":"read-only"},{"id":4,"name":"windows","type":"custom"}]}]}'
            ...
            # Sorted unique across both runners: pscodesign, self-hosted, windows
            @($obj.data.available_tags).Count | Should Be 3
            ($obj.data.available_tags -contains 'self-hosted') | Should Be $true
            ($obj.data.available_tags -contains 'pscodesign')  | Should Be $true
    ...
    Context 'Memory cross-reference' {

        It 'documents the pscodesign label distinction in comments' {
            $content = Get-Content $ProbePath -Raw
            # Plan note: github.com correct label is pscodesign
            ($content -match 'pscodesign') | Should Be $true
        }
    }
}
